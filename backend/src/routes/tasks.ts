 ...
 * Tasks API Routes
 *
 * Provides REST endpoints for Google Tasks API with metadata support.
 */
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const tasks = await getTasksForDateRange(taskListId, startDate as string, endDate as string);
 ...
 * Create a new task with metadata support
 * Body: { title: string, notes?: string, due?: string, parent?: string, assignedUser?: string, assignedUserEmail?: string }
 */
router.post(
  '/tasklists/:taskListId/tasks',
  requireAuth,
  asyncHandler(async (req, res) => {
    const { taskListId } = req.params;
    const { title, notes, due, parent, assignedUser, assignedUserEmail } = req.body;
 ...
 * Update a task
 * Body: { title?: string, notes?: string, due?: string, status?: 'needsAction' | 'completed', assignedUser?: string, assignedUserEmail?: string }
 */
router.patch(
  '/tasklists/:taskListId/tasks/:taskId',
  requireAuth,
  asyncHandler(async (req, res) => {
    const { taskListId, taskId } = req.params;
    const { title, notes, due, status, assignedUser, assignedUserEmail } = req.body;
    ...
    if (assignedUser !== undefined || assignedUserEmail !== undefined) {
      updates.metadata = {};
      if (assignedUser !== undefined) updates.metadata.assignedUser = assignedUser;
      if (assignedUserEmail !== undefined) updates.metadata.assignedUserEmail = assignedUserEmail;
    }
